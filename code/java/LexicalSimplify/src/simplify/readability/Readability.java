...
public final class Readability {
	 ...
	 * @return Fog index.
	 */
	public static float calcFog(final ReadabilityStatsOld stats) {
	 ...
	 */
	public static float calcFlesch(final ReadabilityStatsOld stats) {
		return 206.835f - (1.015f * wordsPerSentence(stats)) - (84.6f * syllablesPerWords(stats));
	}
	
	public static float calcFlesh(String line){		
		return calcFlesch(Fathom.analyze(line));
	}
	 ...
	 */
	public static float calcFleschKincaid(final ReadabilityStatsOld stats) {
		return (11.8f * syllablesPerWords(stats)) + (0.39f * wordsPerSentence(stats)) - 15.59f;
	}
	
	public static float calcFleschKincaid(String line){		
		return calcFleschKincaid(Fathom.analyze(line));
	}

	public static float wordsPerSentence(final ReadabilityStatsOld stats) {
		return ((float) stats.getNumWords()) / stats.getNumSentences();
	}

	public static float percentComplexWords(final ReadabilityStatsOld stats) {
		return (((float) stats.getNumComplexWords()) / stats.getNumWords()) * 100;
	}

	public static float syllablesPerWords(final ReadabilityStatsOld stats) {
