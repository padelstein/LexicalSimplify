...
//			noTargetpr.println(hit.getHITId());
	...
	public void getAssignmentsHIT(String hitId, Map<String, String[]> wordToSense) throws IOException
	{
		try 
		{
			OurHIT currentHIT = new OurHIT(hitId, wordToSense, 50);
	...
	public void getHITs(Map<String, String[]> wordToSense) throws IOException
	{
		try 
		{
			...
			{
				OurHIT currentHIT = new OurHIT(amazonHIT.getHITId(), wordToSense, 50);
				ArrayList<String>  compareList = null;
	...
	public void splitHITs()
	{
		...
		for (int n = 0 ; n < 24 ; n++)
		{
			...
			OurHIT currentContextHIT1 = new OurHIT(currentContextHIT.ID, currentContextHIT.typeID, currentContextHIT.targetWord, currentContextAnswers1, 50);
			OurHIT currentContextHIT2 = new OurHIT(currentContextHIT.ID, currentContextHIT.typeID, currentContextHIT.targetWord, currentContextAnswers2, 50);
			contextHITs1.add(currentContextHIT1);
			contextHITs2.add(currentContextHIT2);
			OurHIT currentNoContextHIT1 = new OurHIT(currentNoContextHIT.ID, currentNoContextHIT.typeID, currentNoContextHIT.targetWord,currentNoContextAnswers1, 50);
			OurHIT currentNoContextHIT2 = new OurHIT(currentNoContextHIT.ID, currentNoContextHIT.typeID, currentNoContextHIT.targetWord,currentNoContextAnswers2, 50);
...
//						System.out.println(input);
					...
					for( int k = 0; k <1000000 && reader.hasNext(); k++ ){ // for counted input
...
//								similarity.println("Overlap, " + contextHit.targetWord + ", " + weightMatched/(double)Math.min(contextHit.answers.size(), noContextHit.answers.size()));
					...
					similarity.close();
						...
						for (int i = 5 ; i < 51 ; i += 5)
						{
							app.contextHITs.clear();
							app.noContextHITs.clear();
						app.fillHitList(new File("/home/ependergast/LexicalSimplify/code/java/LexicalSimplify/contextAnswerOutput.cleaned"), i);
						app.fillHitList(new File("/home/ependergast/LexicalSimplify/code/java/LexicalSimplify/noContextAnswerOutput.cleaned"), i);
//							PrintWriter noContextEntropyOut = new PrintWriter(new FileOutputStream(new File("noContextEntropy"+i+".data.csv")));
...
//								System.out.println("25-25 no context Pearson : " + app.getMirrorPearsonCoeff(app.noContextHITs1, app.noContextHITs2));
//							}
							contextHighFreqData.close();
							noContextHighFreqData.close();
//							noContextEntropyOut.close();
//							contextEntropyOut.close();
						}
...
//					for (int x = 0; x<1000; x++){
					for (OurHIT currentHit: data){
						...
						OurHIT currentHit1 = new OurHIT(currentHit.ID +"1", currentHit.typeID, currentHit.targetWord, current1Answers, 50);
						OurHIT currentHit2 = new OurHIT(currentHit.ID +"2", currentHit.typeID, currentHit.targetWord, current2Answers, 50);
